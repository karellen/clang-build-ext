...
class ClangBuildExtTest(unittest.TestCase):
    ...
    def assert_module_imports_installed(self, ext_name, expected):
        """The wheel's real contract: imports from its install directory, unaided.

        `assert_cxx_module_works` hands the loader an LD_LIBRARY_PATH and imports out of
        the build tree, where a $ORIGIN-relative RUNPATH cannot resolve. Here the
        extension is placed where it would actually be installed and the environment is
        stripped, so only the RUNPATH can satisfy it.
        """
        installed = jp(get_path("platlib"), f"{ext_name}.cpython-"
                       f"{sys.version_info[0]}{sys.version_info[1]}-"
                       f"{os.uname().machine}-linux-gnu.so")
        env = {k: v for k, v in os.environ.items() if k != "LD_LIBRARY_PATH"}

        shutil.copy(self.built_extension(ext_name), installed)
        try:
            result = subprocess.run([sys.executable, "-c",
        ...
        if result.returncode != 0:
            self.fail(f"Installed extension did not import without LD_LIBRARY_PATH:\n"
                      f"stdout: {result.stdout}\nstderr: {result.stderr}")
    ...
    def test_cxx_runpath_resolves_libcxx_without_ld_library_path(self):
        ...
        self.assert_module_imports_installed("test_cxx", "hello, module (deep)")
